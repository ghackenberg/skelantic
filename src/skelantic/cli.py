...
def _load_settings() -> Dict[str, str]:
    settings = {
        ...
        "tests_dir": "tools/skelantic/tests",
        "tests_module": "tools.skelantic.tests"
    }
...
def main() -> None:
    ...
    # RIGHT
    subparsers.add_parser("right", help="Run strict type checks (Pyright).")

    # TEST
    subparsers.add_parser("test", help="Run processor unit tests (Pytest).")

    # VERIFY
    subparsers.add_parser("verify", help="Run full pipeline: generate -> right -> test -> run.")
    ...
    try:
        if args.command == "run":
            ...
            engine.run(".")
            if engine.print_report(): 
                print(f"\n❌ Governance check failed. Run 'skelantic run -v' for detailed traces.")
                sys.exit(1)
            success = True
            next_cmd = None # End of chain
            ...
            if res.returncode == 0:
                ...
                print(f"\n❌ Type check failed. Please fix the Pyright errors.")
                sys.exit(1)

        elif args.command == "test":
            t_dir = settings.get("tests_dir", "tools/skelantic/tests")
            p_dir = settings.get("processors_dir", "tools/skelantic/processors")
        ...
        if success and next_cmd:
            print(f"\n👉 Next recommended step: 'skelantic {next_cmd}'")
