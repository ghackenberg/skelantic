...
class CodeGenerator:
        ...
        def render_node(node_config: Dict[str, Any], path_parts: List[str], class_name: str, parent_class_path: str, is_dir: bool, indent_level: int) -> str:
            ...
            if template_path:
                ...
                if template_path in self.template_models:
                    ...
                    raise ValueError(f"Template-Fehler: Die Datei '{template_path}' (referenziert in config.yaml) wurde nicht im Projekt gefunden. Bitte stelle sicher, dass sie existiert und im Ordner '.skelantic/templates/' liegt.")
    ...
    def generate(self, config_path: str, verbose: bool = False) -> str:
        ...
        # 1. TEMPLATE GRAPH (needed first to check Field usage)
        template_code = self.generate_templates_graph()
        
        # 2. REPOSITORY GRAPH
        repo_code = self.generate_repo_graph(config)
        ...
        # 3. RENDER IMPORTS
        if self._uses_field:
            out += "from pydantic import BaseModel, Field\n"
        else:
